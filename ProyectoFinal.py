...
while opcion != 0:
    ...
    def imprimir(matriz_final):
        for i in range(matriz_final.__len__()): #cantidad de filas
            for j in range(matriz_final[i].__len__()):  #cantidad de columnas
    ...
    def hexagono():
        try:
            input_string = input("'Hexagono'\nIntroduzca el punto inferior izquierdo P0(X0,Y0) y luego\nla medida del lado, separados por espacios: \n\n")
            lista = input_string.split()
            X0 = int(lista[0])
            Y0 = int(lista[1])
            L = int(lista[2])
            lienzoNuevo = recta(X0, Y0, X0+L, Y0)
            lienzoNuevo = recta(X0+L, Y0, X0+1.5*L, Y0 + 0.866*L, lienzoNuevo)
            lienzoNuevo = recta(X0+1.5*L, Y0 + 0.866*L, X0+L, Y0 + 1.732*L, lienzoNuevo)
            lienzoNuevo = recta(X0+L, Y0 + 1.732*L, X0, Y0 + 1.732*L, lienzoNuevo)
            lienzoNuevo = recta(X0, Y0 + 1.732*L, X0-L/2, Y0+0.866*L, lienzoNuevo)
            lienzoNuevo = recta(X0-L/2, Y0+0.866*L, X0, Y0, lienzoNuevo)
            return lienzoNuevo
        except:
            print("\n¡Introduzca valores válidos!\n")
    ...
    def leer():
        ...
        try:
            ...
            dibujo = lienzoNuevo
            print("El archivo '", nombrearchivo, "' fue leído satisfactoriamente")
        except:
